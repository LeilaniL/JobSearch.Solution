    ...
    public static List<IndeedClass> RunSearch(string jobName, string jobLocation)
    {
      ...
      for (int i = 0; i < links.Count - 1; i++)
      {
        ...
        else if (!string.IsNullOrEmpty(links[i].Text))
        {
          ...
          {
            //   prints error message in details box if details couldn't be located
            tempCompany = "";
            tempLocation = "";
            tempDate = "";
            tempDescription = "Sorry, an error occured. Please try again.";
            IndeedClass errorMsg = new IndeedClass(tempTitle, tempLink, tempCompany, tempLocation, tempDate, tempDescription);
            indeedJobs.Add(errorMsg);
            return indeedJobs;
          }
        }
